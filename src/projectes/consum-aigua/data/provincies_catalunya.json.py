...
files_shape_selected = [file_name for file_name in zip_file_memory.namelist() if 'provincies-1000000' in file_name]
...
provincies_df = gpd.read_file(shapefile_path)
sys.stderr.writelines([str(len(provincies_df.geometry))])

# Convertir a CRS estàndard (EPSG 4326)
provincies_df = provincies_df.to_crs(epsg=4326)
sys.stderr.writelines([str(len(provincies_df.geometry))])

# Guardar el fitxer GeoJSON al directori temporal
output_file = os.path.join("./", "provincies_catalunya.json")
provincies_df.to_file(output_file, driver="GeoJSON")
