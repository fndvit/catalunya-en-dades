...
temporal_directory = tempfile.mkdtemp()
...
for level_map_file in files_shape_selected:
    with zip_file_memory.open(level_map_file) as file:
        with open(f'{temporal_directory}/{level_map_file}', 'wb') as f:
            f.write(file.read())

comarques_df = gpd.read_file(
    f'{temporal_directory}/{next(file_name for file_name in files_shape_selected if file_name.endswith('.shp'))}')
